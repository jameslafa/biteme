...
test.describe('What\'s New Page', () => {
  ...
  test('entries show day circle and text', async ({ page }) => {
    ...
    await expect(firstEntry.locator('.timeline-entry-text')).toContainText('Recipe recommendations');
  });
  ...
  test('newest entry is first', async ({ page }) => {
    ...
    expect(firstText).toContain('Recipe recommendations');
